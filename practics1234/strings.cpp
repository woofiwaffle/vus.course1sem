...
string text(){
    ...
    if(n == 1){
        ...
        if (File.is_open()) {
            while (!File.eof()) {  /* eof - returns true if the file is empty, and returns false if the file is not empty
                                           (выдает истину, если файл пуст, а если файл не пуст, то выдает ложь)*/
                string s1;
                getline(File, s1);
                s = s + s1;
            }
...
string charact(string s){           /*  counted together with spaces and dots
                                       (считается вместе с пробелами и точками)*/

    int i = 0;
    if(s[i] == ' '){
        while(s[i] != '\0') {       /* "\0" - means end of line
...
string convertion(string s) {
    ...
    bool a = true;
    for(int i = 0; i < s.length(); i++){    // s.length() - длина строки
        if(s[i] == ' '){
            if(a){
                continue;
            }
